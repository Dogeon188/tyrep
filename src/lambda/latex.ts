...
function binderSubAt(ctx: Ctx, i: number, binderSubs: Map<string, number>): number | undefined {
...
function termToString(t: Term, ctx: Ctx, binderSubs: Map<string, number>): string {
  switch (t.kind) {
    case 'var':
      return varLatex(t.name, binderSubAt(ctx, lookupIndex(ctx, t.name), binderSubs))
    case 'prim':
      return `\\texttt{${t.name}}`
    case 'lit':
    ...
    case 'abs': {
      ...
      const param = varLatex(t.param, extended !== ctx ? binderSubs.get(JSON.stringify(extended)) : undefined)
      return `λ${param}. ${termToString(t.body, extended, binderSubs)}`
    }
    case 'app': {
      const fn = t.fn.kind === 'abs' ? `(${termToString(t.fn, ctx, binderSubs)})` : termToString(t.fn, ctx, binderSubs)
      const arg = t.arg.kind === 'var' ? termToString(t.arg, ctx, binderSubs) : `(${termToString(t.arg, ctx, binderSubs)})`
      return `${fn} ${arg}`
    }
  }
}
...
function typeToLatex(t: Type): string {
  ...
  const from = t.from.kind === 'arrow' ? `(${typeToLatex(t.from)})` : typeToLatex(t.from)
...
function ctxLatex(ctx: Ctx, binderSubs: Map<string, number>): string {
  return ctx
    .map(([n, t], i) => `${varLatex(escape(n), binderSubAt(ctx, i, binderSubs))} : ${typeToLatex(t)}`)
    .join(', ')
}

function envLatex(ctx: Ctx, labels: Map<string, number>, binderSubs: Map<string, number>): string {
...
function judgment(n: ProofNode, labels: Map<string, number>, binderSubs: Map<string, number>): string {
  return `${envLatex(n.ctx, labels, binderSubs)} \\vdash ${escape(termToString(n.term, n.ctx, binderSubs))} : ${typeToLatex(n.type)}`
}

function nodeToLatex(n: ProofNode, labels: Map<string, number>, binderSubs: Map<string, number>): string {
  ...
  const premiseLatex = n.premises.map((p) => nodeToLatex(p, labels, binderSubs)).join('\n')
...
export function proofToLatex(root: ProofNode): string {
  ...
  const legend = entries.map((ctx, i) => `\\[\\Gamma_{${i}} = ${ctxLatex(ctx, binderSubs)}\\]`)

  return [...legend, '\\begin{prooftree}', nodeToLatex(root, labels, binderSubs), '\\end{prooftree}'].join('\n')
}
